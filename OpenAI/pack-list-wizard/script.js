const url = "ZUPLO URL"; //url for zuplo api
...
/**Get the total weight of a category and convert it to pounds if neccesary */
function extractWeightInPounds(weightString) {
    // Define a regular expression pattern to match the input string
    const pattern = /^(\d*\.?\d+)\s*?(pounds?|lbs?|ounces?|oz)$/i;

    // Use the regular expression to match and extract the number and unit
    const match = weightString.match(pattern);

    if (match) {
        const value = parseFloat(match[1]); // Extracted numeric value
        const unit = match[2].toLowerCase(); // Extracted unit, converted to lowercase

        if (unit === 'ounces' || unit === 'oz') {
            return value / 16; // Convert ounces to pounds (1 pound = 16 ounces)
        } else if (unit === 'pounds' || unit === 'pound' || unit === 'lbs' || unit === 'lb') {
            return value; // Return value as is for pounds or its variations
        } else {
            // Handle other unit variations or unknown units
            return null;
        }
    } else {
...
function extractPriceInDollars(text) {
...
async function checkMiscWeight(miscCategory, hikeLength) {
    ...
    if (isLessThan(miscWeight, 0.8 * miscValue)) {
        ...
        userQuestion += generateCategoryPrompt(key, hikeLength, value);
        console.log(userQuestion);
